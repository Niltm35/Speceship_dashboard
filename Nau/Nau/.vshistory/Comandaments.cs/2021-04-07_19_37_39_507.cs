        ...
        private void descodificacion_zip()
        {
            ...
            if (acabado) {
                for (int i = 1; i < 4; i++)
                {
                    ...
                    string FinalTxt = Application.StartupPath + @"\Fitxers\PACS_desc2.txt";

                    FileStream fs = new FileStream(FinalTxt, FileMode.OpenOrCreate, FileAccess.Write);
                    //StreamWriter writer = new StreamWriter(fs);
                    //StreamWriter writer2 = File.AppendText(FinalTxt);

                    //writer2.Write(text);
                    


                    if (!File.Exists(FinalTxt))
                    {
                    ...
                    using (StreamWriter writer = File.AppendText(FinalTxt))
                    {
                        writer.Write(text);
                        writer.Close();
                    }
