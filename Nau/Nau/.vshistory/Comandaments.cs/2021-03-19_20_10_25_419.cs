...
namespace Nau
{
    public partial class Comandaments : Form
    {
        ...
        private void SendVK(byte[] cod_VK)
        {
            TcpClient client = new TcpClient(IP_Planet, Convert.ToInt32(Port1_Planet));
            NetworkStream ns_vk = client.GetStream();

            byte[] SendingBuffer = null;
            var BufferSize = 50024;

            int NoOfPackets = Convert.ToInt32
            (Math.Ceiling(Convert.ToDouble(cod_VK.Length) / Convert.ToDouble(BufferSize)));
            int TotalLength = cod_VK.Length, CurrentPacketLength;
            ...
            for (int i = 0; i < NoOfPackets; i++)
            {
                ...
                ns_vk.Write(SendingBuffer, 0, (int)SendingBuffer.Length);
            }
            ns_vk.Close();
            lbx_Missatges.Items.Add("YOU: " + cod_VK.ToString());
        }
        ...
        public void sendPort1()
        {
            ...
            string zipFile = Application.StartupPath + @"\Fitxers\PACS_m.rar";
            byte[] SendingBuffer = null;
            var BufferSize = 50024;
        ...
        public void conexionPort1()
        {
            ...
            while (IsConnected)
            {
                if (Listener2.Pending())
                {
                    ...
                    string zipFile = Application.StartupPath + @"\Fitxers\PACS.rar";
                    //------------------------------recibir fichero----------------------
        ...
        private void comboPlanet_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            Name_Planet = TaulaPlanet().Tables[0].Rows[0][10].ToString();

        }

        private void pictureBox1_Click(object sender, EventArgs e)
        {
            BackgroundImage = Image.FromFile(Application.StartupPath + @"\imatges\planets" + Name_Planet );
        }
