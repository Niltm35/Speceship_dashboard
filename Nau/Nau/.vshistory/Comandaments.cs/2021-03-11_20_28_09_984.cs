...
namespace Nau
{
    public partial class Comandaments : Form
    {
        ...
        private void Comandaments_Load(object sender, EventArgs e)
        {
            ...
            string pathApplication = System.IO.Path.Combine(AppDomain.CurrentDomain.BaseDirectory, @"..\Nau\imatge\server_on.png");
            img_server.BackgroundImage = Image.FromFile(pathApplication + @"Nau\imatge\server_on.png");
        ...
        public void message_planet(string ms)
        {
            ...
            if (request == "VR")
            {
                if (answer == "VP")
                {
                    MessageBox.Show("Validation ln Progress");
                }
                else if (answer == "AD")
                {
                ...
                {
                    MessageBox.Show("Error message: " + ms);
                }
            }
        }
